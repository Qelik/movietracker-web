...
const watchlistCache = { value: null, loading: null };
const diaryCache = { value: null, loading: null };
const ratingsCache = { value: null, loading: null };
const tagsCache = { value: null, loading: null };
function through(cache, load) {
    if (cache.value)
...
export function invalidate(what = ['watchlist', 'diary', 'ratings', 'tags']) {
    if (what.includes('watchlist'))
        watchlistCache.value = null;
    if (what.includes('diary'))
        diaryCache.value = null;
    if (what.includes('ratings'))
        ratingsCache.value = null;
    if (what.includes('tags'))
...
export function key(mediaType, tmdbId) {
    return `${mediaType}:${tmdbId}`;
}
export async function stateFor(mediaType, tmdbId) {
...
export async function watchedKeys() {
    const diary = await getDiary();
    return new Set(diary.map((entry) => key(entry.mediaType, entry.tmdbId)));
}
export async function watchlistKeys() {
