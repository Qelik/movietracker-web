...
async function load(body, redraw) {
    clear(body);
...
function row(entry, rating, redraw) {
    const poster = posterUrl(entry.movie?.posterPath ?? null, 'w185');
    const open = () => openTitle({ mediaType: entry.mediaType, tmdbId: entry.tmdbId, title: entry.movie?.title });
...
function openEdit(entry, redraw) {
