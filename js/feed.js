...
export async function renderFeed(body) {
    clear(body);
    ...
    if (page.items.length === 0) {
        body.append(empty(page.sharingCount === 0
 ...
 */
export function feedRow(item, withAuthor) {
    ...
    row.addEventListener('click', () => openTitle({ mediaType: item.mediaType, tmdbId: item.tmdbId, title }));
    return row;
}
