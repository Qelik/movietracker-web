...
export function openTitle(ref: TitleRef | MovieSummary): void {
...
async function render(body: HTMLElement, ref: TitleRef, close: () => void): Promise<void> {
  let detail: MovieDetail
  let state: library.TitleState
  let allTags: Tag[]
  let titleTags: Tag[]
  ...
  const paint = () => {
    ...
    add(
      ...
      tagsBlock(detail, allTags, titleTags, redraw),
      castBlock(detail),
...
function heroFor(detail: MovieDetail): HTMLElement {
  const backdrop = backdropUrl(detail.backdropPath)
  const poster = posterUrl(detail.posterPath, 'w342')

  const facts: string[] = []
  if (detail.releaseYear) facts.push(String(detail.releaseYear))
  ...
  if (detail.seriesStatus) facts.push(detail.seriesStatus)
...
function actionsFor(
  ...
  const watchlistButton = button(
    ...
    () =>
      void busy(watchlistButton, async () => {
        try {
          if (onWatchlist) {
  ...
  const logButton = button(
    detail.mediaType === 'tv' ? 'Log episode' : 'Log viewing',
    () => openLogSheet(detail, redraw),
    'primary',
  )
...
function viewingRow(entry: DiaryEntry, redraw: () => Promise<void>): HTMLElement {
  const label =
...
function openEditViewing(entry: DiaryEntry, redraw: () => Promise<void>): void {
...
function openLogSheet(detail: MovieDetail, redraw: () => Promise<void>): void {
  ...
  const quick = el('div', { class: 'row-actions' }, [
    ...
    detail.releaseDate
      ? button(`Release day (${detail.releaseDate})`, () => {
          date.value = detail.releaseDate as string
        }, 'ghost small')
      : null,
  ])
...
function seasonsBlock(detail: MovieDetail, redraw: () => Promise<void>): HTMLElement {
...
function similarBlock(detail: MovieDetail): HTMLElement {
  ...
  void api
    ...
    .then((page) => {
      ...
      for (const movie of results) rail.append(posterCard(movie, openTitle))
    })
    .catch(() => {
      clear(rail)
...
function openAddToList(detail: MovieDetail): void {
  ...
  void api
    ...
    .then(({ owned, shared }) => {
      ...
      if (writable.length === 0) {
        body.append(
          el('p', { class: 'muted', text: 'You have no lists you can add to. Make one on the Lists tab.' }),
        )
        return
      }

      const picker = select(
        writable.map((list) => ({ value: list.id, label: list.name })),
        writable[0]?.id ?? '',
      )
