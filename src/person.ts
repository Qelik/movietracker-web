...
import type { PersonCredit, PersonPage } from './types.js'

/** Which slice of the filmography is showing. */
type Lens = 'all' | 'acting' | 'crew' | 'seen' | 'unseen'

export function openPerson(tmdbId: number, name?: string): void {
...
function header(page: PersonPage): HTMLElement {
  const { person, seen } = page
  const portrait = posterUrl(person.profilePath, 'w185')

  const facts: string[] = []
  if (person.knownForDepartment) facts.push(person.knownForDepartment)
  if (person.birthday) {
...
function seenLine(seen: { watched: number; total: number }): string {
  if (seen.total === 0) return 'No credits on TMDB.'
  if (seen.watched === 0) return `You have not logged any of these ${seen.total} credits.`
  return `You have watched ${seen.watched} of ${seen.total} credits.`
}

function lensBar(page: PersonPage, lens: Lens, pick: (lens: Lens) => void): HTMLElement | null {
  const hasActing = page.credits.some((credit) => credit.isCast)
  const hasCrew = page.credits.some((credit) => !credit.isCast)

  const options: { value: Lens; label: string }[] = [{ value: 'all', label: 'Everything' }]
...
function applyLens(credits: PersonCredit[], lens: Lens): PersonCredit[] {
  switch (lens) {
    case 'acting':
...
function filmography(page: PersonPage, lens: Lens): HTMLElement {
...
function creditRow(credit: PersonCredit): HTMLElement {
  const poster = posterUrl(credit.posterPath, 'w185')

  const meta: string[] = []
  if (credit.releaseYear) meta.push(String(credit.releaseYear))
  if (credit.roles.length) meta.push(credit.roles.join(', '))
  if (credit.episodeCount) meta.push(plural(credit.episodeCount, 'episode'))
