...
function card(list: ListSummary, body: HTMLElement): HTMLElement {
...
function openCreate(done: () => void): void {
...
export function openList(id: string, done: () => void): void {
...
function header(
  ...
  if (list.role === 'owner') {
    ...
    const remove = button('Delete list', () =>
...
function canEdit(role: ListRole): boolean {
...
function itemsBlock(
  ...
  for (const item of items) block.append(itemRow(list, item, refresh))
  return block
}

function itemRow(list: ListSummary, item: ListItem, refresh: () => Promise<void>): HTMLElement {
  const poster = posterUrl(item.movie?.posterPath ?? null, 'w185')

  const open = () =>
    openTitle({ mediaType: item.mediaType, tmdbId: item.tmdbId, title: item.movie?.title })
  ...
  for (const option of options) {
    ...
    const chip = button(
      option.label,
      () =>
        void busy(chip, async () => {
          try {
            if (chosen) await api.clearVote(list.id, item.id)
            else await api.voteOn(list.id, item.id, option.value)
            await refresh()
          } catch (error) {
            toast(message(error), 'error')
          }
  ...
  const remove = canEdit(list.role)
    ? button('Remove', () =>
        void busy(remove as HTMLButtonElement, async () => {
          try {
...
function openAddTitle(list: ListSummary, refresh: () => Promise<void>): void {
...
function openRename(list: ListSummary, refresh: () => Promise<void>, done: () => void): void {
...
function openSharing(list: ListSummary, refresh: () => Promise<void>, done: () => void): void {
  ...
  const paint = (code: string | null) => {
    ...
    const mint = button(code ? 'Replace code' : 'Create code', () =>
      void busy(mint, async () => {
        try {
    ...
    const revoke = code
      ? button('Revoke', () =>
          void busy(revoke as HTMLButtonElement, async () => {
            try {
...
function membersBlock(list: ListSummary, refresh: () => Promise<void>): HTMLElement {
  ...
  if (list.role === 'owner') {
    ...
    const send = button('Invite', () =>
      void busy(send, async () => {
        const value = handle.value.trim().toLowerCase().replace(/^@/, '')
        if (!value) return
        try {
          await api.invite(list.id, value, role.value as 'editor' | 'viewer')
          handle.value = ''
          toast('Invited')
          await refresh()
        } catch (error) {
      ...
      for (const member of members) {
        ...
        if (list.role === 'owner' && member.role !== 'owner') {
          ...
          roleSelect.addEventListener('change', () => {
            void api
              .setMemberRole(list.id, member.userId, roleSelect.value as 'editor' | 'viewer')
              .then(() => toast('Role updated'))
              .catch((error: unknown) => toast(message(error), 'error'))
          })
          controls.push(roleSelect)

          const kick = button('Remove', () =>
            void busy(kick, async () => {
              if (!(await confirmModal({
                title: 'Remove person',
