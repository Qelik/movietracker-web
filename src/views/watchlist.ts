...
import type { UpNextShow, WatchlistItem } from '../types.js'
 ...
 */
let providerFilter: number | null = null
...
async function load(
  ...
  try {
    ...
    if (picker) services.append(picker)

    const shown =
      providerFilter === null
        ? byMedia
        : byMedia.filter((item) =>
            item.watchProviders?.subscription.some(
    ...
    if (shown.length === 0) {
      ...
      if (unchecked > 0) body.append(uncheckedNote(unchecked))
      return
    }
 ...
 * label asks.
 */
function servicePicker(items: WatchlistItem[], redraw: () => void): HTMLElement | null {
...
function uncheckedNote(count: number): HTMLElement {
  return el('p', {
    class: 'muted',
 ...
 */
async function loadUpNext(host: HTMLElement): Promise<void> {
...
function upNextCard(show: UpNextShow): HTMLElement {
  const poster = posterUrl(show.posterPath, 'w185')
  const code = `S${show.next.seasonNumber}E${show.next.episodeNumber}`

  const facts: string[] = [code]
  if (show.next.name) facts.push(show.next.name)
  ...
  const open = button(
    'Details',
    () => openTitle({ mediaType: 'tv', tmdbId: show.tmdbId, title: show.title }),
    'ghost small',
  )
  ...
  card.append(
    el('div', { class: 'row-actions' }, [show.status === 'ready' ? watched : null, open]),
  )
  return card
}
 ...
 */
function showProgress(show: UpNextShow): HTMLElement {
  const aired = show.airedEpisodes
  const done = aired === 0 ? 0 : show.watchedEpisodes / aired

  return progressBar(
    done,
    `${show.watchedEpisodes}/${aired}`,
    `${show.title}: ${show.watchedEpisodes} of ${plural(aired, 'aired episode')} watched`,
  )
}
