...
export function openProfile(handle: string): void {
  const body = el('div', { class: 'person-sheet' }, [spinner('Loading')])
  modal({ title: `@${handle}`, body, wide: true })
  void render(body, handle)
}

async function render(body: HTMLElement, handle: string): Promise<void> {
...
function followButton(view: ProfileView, body: HTMLElement, handle: string): HTMLElement {
  const following = view.isFollowing === true

  const node = button(
    following ? 'Following ✓' : 'Follow',
    () =>
      void busy(node, async () => {
        try {
          if (following) {
